...
class FileManager:
    ...
    def list_permissions():
        """Lists files in the current directory with permission details for the logged-in user."""
        if current_user is None:
            print("❌ Error: No user logged in.")
            return

        print(f"📂 Listing files with permissions for {current_user['role']}:")
        
        files = os.listdir(os.getcwd())  # List files in the current directory
        for filename in files:
            readable = FileManager.check_permission(filename, "read")
            writable = FileManager.check_permission(filename, "write")
    ...
    def run():
        ...
        while True:
            try:
                ...
                # --- New Command: List File Permissions ---
                if cmd_input == "list_permissions":
                    FileManager.list_permissions()
                    continue
